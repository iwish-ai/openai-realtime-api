...
export class RealtimeAPI extends RealtimeEventHandler {
  ...
  receive(eventName, event) {
    ...
    // Debug: log session.updated to verify transcription config accepted
    if (eventName === 'session.updated') {
      // console.log('[DEBUG API] session.updated - full session:', JSON.stringify({
      //   input_audio_transcription: event?.session?.input_audio_transcription,
      //   audio: event?.session?.audio,
      //   type: event?.session?.type,
      // }, null, 2));
    }
    // Debug: log any transcription-related events
    if (eventName.includes('transcription')) {
      // console.log('[DEBUG API] transcription event:', eventName, event);
    }
  ...
  send(eventName, data) {
    ...
    // log event and data
    // console.log(`[DEBUG API] sent:`, eventName, event);
    return true;
  }
}

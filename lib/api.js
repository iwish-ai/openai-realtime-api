...
export class RealtimeAPI extends RealtimeEventHandler {
  /**
   * Normalizes api key input so callers can pass either:
   * - raw key string (`ek_...`)
   * - axios/fetch style response envelope from chat/getRealTimeChatSession
   * @param {any} apiKey
   * @returns {string | null}
   */
  static resolveApiKey(apiKey) {
    if (!apiKey) return null;
    if (typeof apiKey === 'string') return apiKey;

    const nestedKey =
      apiKey?.data?.data?.client_secret?.value ??
    ...
    return typeof nestedKey === 'string' ? nestedKey : null;
  }
  ...
  constructor({ url, apiKey, dangerouslyAllowAPIKeyInBrowser, debug } = {}) {
    ...
    this.apiKey = RealtimeAPI.resolveApiKey(apiKey);
    this.debug = !!debug;
    this.ws = null;
  ...
  async connect({ model } = { model: 'gpt-realtime-2' }) {
    ...
    if (globalThis.WebSocket) {
      ...
      const browserProtocols = ['realtime'];
      if (this.apiKey) {
        browserProtocols.push(`openai-insecure-api-key.${this.apiKey}`);
      }
